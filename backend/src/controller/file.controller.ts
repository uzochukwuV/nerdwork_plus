...
const generateS3Key = (userId: string, filename: string, comicId?: string): string => {
...
export const getComicFileInfo = async (req: any, res: any) => {
  try {
    ...
    // Get file metadata
    const headCommand = new GetObjectCommand({ Bucket: BUCKET_NAME, Key: s3Key });
    ...
    let signedUrl = null;
    if (!isPublic) {
      signedUrl = await getSignedUrl(s3Client, headCommand, { expiresIn: 3600 });
    }
