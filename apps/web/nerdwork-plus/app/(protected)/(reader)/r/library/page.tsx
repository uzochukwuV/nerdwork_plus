...
const LibraryPage = () => {
  ...
  const [showFree, setShowFree] = React.useState<Checked>(true);
  ...
  const filteredComics = React.useMemo(() => {
    let tempComics = [...comics]; // Start with a copy of the original data
    ...
    if (showFree) {
      tempComics = tempComics.filter((comic) => !comic.isPaid); // Assumes comic.isPaid is a boolean
    } else if (showPaid) {
